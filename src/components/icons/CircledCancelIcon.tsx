// components/icons/CircledCancelIcon.tsx

import React from 'react';

interface CircledCancelIconProps {
    width?: number;
    height?: number;
    color?: string;
    className?: string;
}

const CircledCancelIcon: React.FC<CircledCancelIconProps> = ({
    width = 24,
    height = 24,
    color = '#E0E0E0',
    className = '',
}) => (
    <svg
        width={width}
        height={height}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className={className}
    >
        <path
            d="M14.9 22.75H9.10001C8.21001 22.75 7.07 22.28 6.45 21.65L2.35001 17.55C1.72001 16.92 1.25 15.78 1.25 14.9V9.10001C1.25 8.21001 1.72001 7.07001 2.35001 6.45001L6.45 2.35001C7.08 1.72001 8.22001 1.25 9.10001 1.25H14.9C15.79 1.25 16.93 1.72001 17.55 2.35001L21.65 6.45001C22.28 7.08001 22.75 8.22001 22.75 9.10001V14.9C22.75 15.79 22.28 16.93 21.65 17.55L17.55 21.65C16.92 22.28 15.79 22.75 14.9 22.75ZM9.10001 2.75C8.61001 2.75 7.84999 3.06 7.50999 3.41L3.41 7.51001C3.07 7.86001 2.75 8.61001 2.75 9.10001V14.9C2.75 15.39 3.06 16.15 3.41 16.49L7.50999 20.59C7.85999 20.93 8.61001 21.25 9.10001 21.25H14.9C15.39 21.25 16.15 20.94 16.49 20.59L20.59 16.49C20.93 16.14 21.25 15.39 21.25 14.9V9.10001C21.25 8.61001 20.94 7.85001 20.59 7.51001L16.49 3.41C16.14 3.07 15.39 2.75 14.9 2.75H9.10001Z"
            fill={color}
        />
        <path
            d="M8.49945 16.25C8.30945 16.25 8.11945 16.18 7.96945 16.03C7.67945 15.74 7.67945 15.26 7.96945 14.97L14.9695 7.97C15.2595 7.68 15.7395 7.68 16.0295 7.97C16.3195 8.26 16.3195 8.74 16.0295 9.03L9.02945 16.03C8.87945 16.18 8.68945 16.25 8.49945 16.25Z"
            fill={color}
        />
        <path
            d="M15.4995 16.25C15.3095 16.25 15.1195 16.18 14.9695 16.03L7.96945 9.03C7.67945 8.74 7.67945 8.26 7.96945 7.97C8.25945 7.68 8.73945 7.68 9.02945 7.97L16.0295 14.97C16.3195 15.26 16.3195 15.74 16.0295 16.03C15.8795 16.18 15.6895 16.25 15.4995 16.25Z"
            fill={color}
        />
    </svg>
);

export default CircledCancelIcon;
